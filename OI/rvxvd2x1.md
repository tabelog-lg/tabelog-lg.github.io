...
public:
    ...
    {
        bign::ACCURACY = accuracy;
    }

#define bign_ZERO bign::ZERO()
#define bign_ONE bign::ONE()
#define bign_TEN bign::TEN()

private:
    static int ACCURACY{100}; // 除法精度
...
bign::bign(const string &num) // 用字符串初始化，格式形如"-123.456"、"1.0"
{
    // 用于判断小数与整数部分交界
...
bign::bign(const bign &num) // 利用高精度类初始化
{
    integer = num.integer;
    decimal = num.decimal;
    tag = num.tag;
...
bign::bign(bign &&num) noexcept // 移动构造
{
...
bign bign::operator =(const bign &num) // 赋值（拷贝）操作
{
    integer = num.integer;
    decimal = num.decimal;
    tag = num.tag;
...
int bign::toInt() const // 转换为int
{
    return stoi(toString());
}

long bign::toLong() const // 转换为long
{
    return stol(toString());
}

long long bign::toLongLong() const // 转换为long long
{
    return stoll(toString());
}

float bign::toFloat() const // 转换为float
{
    return stof(toString());
}

double bign::toDouble() const // 转换为double
{
    return stod(toString());
}

long double bign::toLongDouble() const // 转换为long double
{
    return stold(toString());
}

bign operator - (const bign &num) // 取负操作
{
    bign temp(num);
    temp.tag = !temp.tag;
    return temp;
...
ostream &operator << (ostream &out, const bign &num) // 输出重载
{
    if (!num.tag) // 负数
    {
        out << "-";
...
istream &operator >> (istream &in, bign &num) // 输入重载
{
    string str;
    in >> str;
    num = bign(str);
...
bign operator += (bign &num1, const bign &num2) // 加等于重载
{
...
bign operator -= (bign &num1, const bign &num2) // 减等于重载
{
...
bign operator *= (bign &num1, const bign &num2) // 乘等于重载
{
    bign result(0);                               // 储存结果
...
bign operator /= (bign &num1, const bign &num2) // 除等于重载
{
    if (num2 == bign_ZERO)
...
bign operator + (const bign &num1, const bign &num2) // 调用+=
{
    bign temp(num1);
    temp += num2;
    return temp;
...
bool operator < (const bign &num1, const bign &num2) // 小于重载
{
    bool sign;                // 返回值
...
bool operator > (const bign &num1, const bign &num2) // 大于重载
{
    bool tag = !(num1 <= num2);
    return tag;
}

bool operator == (const bign &num1, const bign &num2) // 等于重载
{
...
测试程序 `_test_bignum_cal.cpp`
```cpp
// calculate pi
#include <chrono>
#include "bignum.h"

bign pi(int iter, int accuracy = -1)
{
    if (accuracy != -1)
        bign::setAccuracy(accuracy);
...
```
测试程序 `_test_bignum_type.cpp`
```cpp
// type conversation
#include <iostream>
#include "bignum.h"

void TypeConversion(const bign &num)
{
    std::cout << "num              = " << num << endl;
    std::cout << "num.toString     = " << num.toString() << endl;
    std::cout << "num.toFloat      = " << num.toFloat() << endl;
    std::cout << "num.toDouble     = " << num.toDouble() << endl;
    std::cout << "num.toLongDouble = " << num.toLongDouble() << endl;
    std::cout << "num.toInt        = " << num.toInt() << endl;
    std::cout << "num.toLong       = " << num.toLong() << endl;
    std::cout << "num.toLongLong   = " << num.toLongLong() << endl;
}
